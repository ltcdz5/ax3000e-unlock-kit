...
| 设备 / 拓扑 | 状态 | 说明 |
|---|---|---|
| AX3000E（RN07）固件 1.0.24 | ✅ 全支持（实测基准） | 双模式面板 / 三层自愈 / DNS 去广告，本套件参照实现 |
| 同代 IPQ 平台（AX3000T 等） | ⚠️ 思路一致 | 同为 start_binding 注入解锁，需实机校准 |
| BE3600 2.5G | ⏳ 预适配 | 见 `docs/BE3600-解锁与适配指南.md`，待实机验证 |
| 小米 Mesh（无线/有线回程） | ⏳ 预设计 | 见 `docs/Mesh拓扑适配笔记.md`，去广告挂主节点 |
...
这是**单设备、单固件版本的运维工具包**，不是通用固件方案：绑定 AX3000E（RN07）官方固件 1.0.24，核心价值是 SSH 解锁 + 三层自愈、DNS/去广告定制、本地 Web 面板。几条如实的边界：

- **不是刷机项目**：不刷 OpenWrt / ImmortalWrt，全程保留小米官方固件，只在原厂系统上做 SSH 解锁与配置定制——要刷机请另寻教程；但只要 DNS/去广告/自愈这些需求，保留原厂固件 + 本套件是风险更低的路（BE3600 同理，见其适配指南）
...
| 08-28 | v3.0.0 | 架构统一：monitor_web 成为双面板唯一服务层，主面板删 185 行重复实现；/api/act 表单+JSON 双解析（修复浏览器按钮回归）；新增 14 项本地单测进 CI |
| 08-28 | v3.0.1→v3.1.0 | 应答格式与解析判定同源（修 curl 302 空响应）；表单报错 302 友好回跳；**DNS 查询日志可开关**（降 dnsmasq CPU）；anti-AD 列表转 `/#` 格式（实测本机 dnsmasq 对 `/#` 仍回 0.0.0.0，无行为差异，如实记录）；主面板交互统一为 fetch+toast+局部刷新，砍掉服务端双重渲染与 ?msg= 整页刷新 |

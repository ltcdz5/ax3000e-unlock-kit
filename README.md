...
| 08-28 | v3.0.1→v3.1.0 | 应答格式与解析判定同源（修 curl 302 空响应）；表单报错 302 友好回跳；**DNS 查询日志可开关**（降 dnsmasq CPU）；anti-AD **NXDOMAIN 化**（客户端不再重试）；主面板交互统一为 fetch+toast+局部刷新，砍掉服务端双重渲染与 ?msg= 整页刷新 |

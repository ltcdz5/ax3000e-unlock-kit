...
> **验证范围**：仅 AX3000E（RN07）固件 1.0.24 全套实测通过；上表未列的机型/固件**均未验证**，思路可参考但接口与路径需你自行校准。跑通了别的机型、或遇到问题，欢迎开 [Bug 反馈](../../issues/new?template=bug-report.md) / [兼容性报告](../../issues/new?template=compatibility-report.md)——真机回报是本项目最缺、也最有价值的输入。

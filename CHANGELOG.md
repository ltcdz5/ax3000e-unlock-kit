...
### 机型适配
- 实测基准固件版本按设备型号从能力表取用，面板体检与一键体检不再假定接入的是 AX3000E；型号未收录时如实报告「无基准可比」，不借用其它型号的版本
- 型号标识忽略大小写匹配：管理页指纹与 `nvram get model` 对同一设备返回的大小写不同，已知型号不会被误判成未收录

### 一键体检
- 去广告链路改为实测拦截行为：抽样解析规则列表内的域名，按落黑洞的比例判定，主路由与 AP 中继两种拓扑下结论一致
- 发现路由器时扫描本机全部网段，电脑开着 VPN 或多网卡时照样能找到设备

### 迁移包
- 导出前即创建输出目录，路径不可用会第一时间报错，不会跑完采集才失败在最后一步
- 机型、固件、自愈脚本三项信息改用实测有效的读取方式，导出报告如实反映设备状态

### 防误升级
- 通过固件自带的升级预检开关在每次开机压实自动升级关闭，防护真实生效
- 一键体检可检测该项是否被改回并自动纠正
